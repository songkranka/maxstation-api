...
namespace DailyOperation.API.Services
{
    public static class DefaultService
    {
        ...
        public static IActionResult DoAction<T>(string pStrFunctionName, Func<T> pFunc)
        {
            try
            {
                T result;
        ...
        public static int GetInt(object pInput, int pIntDefault = 0)
        {
            if (pInput == null || Convert.IsDBNull(pInput))
            {
                return pIntDefault;
            }
            int result = pIntDefault;
            if( int.TryParse(GetString(pInput),out result))
            {
                return result;
            }
            else
        ...
        public static string EncodeSqlDate(DateTime pInput)
        {
            return pInput.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
        }
