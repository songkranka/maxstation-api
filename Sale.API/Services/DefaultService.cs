...
namespace Sale.API.Services
{
    public static class DefaultService
    {
        ...
        public static async Task<T> ExecuteScalar<T>(PTMaxstationContext pContext, string pStrSql)
        {
            if(pContext == null)
            {
                return default(T);
            ...
            T result = default(T);
            var con = pContext.Database.GetDbConnection();            
        ...
        {
            var con = pContext?.Database?.GetDbConnection();
            if(con == null || !(con is SqlConnection))
            {
                return null;
            }
            pStrSql = GetString(pStrSql);
            ...
            DataTable result = new DataTable();
            using (var da = new SqlDataAdapter(pStrSql, con as SqlConnection))
            {
                await Task.Run(() => da.Fill(result));
            }
            return result;
