...
namespace Inventory.API.Services
{
    public static class DefaultService
    {
        ...
        public static decimal GetDecimal(object pInput, decimal pDecDefault = 0)
        {
            if (pInput == null || Convert.IsDBNull(pInput))
            {
                return pDecDefault;
            }
            decimal result = pDecDefault;
            if (decimal.TryParse(GetString(pInput), out result))
            {
                return result;
            }
            else
        ...
        public static string EncodeSqlDateTime(DateTime pInput)
        {
            return pInput.ToString("yyyy-MM-dd hh:mm:ss", CultureInfo.InvariantCulture);
        }
        ...
        public static async Task<DataTable> GetDataTable(PTMaxstationContext pContext, string pStrSql)
        {
        ...
        public static async Task<DataTable> GetDataTable(PTMaxstationContext pContext, string pStrSql , SqlParameter[] pArrParam = null)
        {           
            var conDb = pContext.Database.GetDbConnection();
            if (!(conDb is SqlConnection))
            {
            ...
            var com = conDb.CreateCommand() as Microsoft.Data.SqlClient.SqlCommand;
            com.CommandText = pStrSql;
            if(pArrParam != null  && pArrParam.Length > 0)
            {
                com.Parameters.AddRange(pArrParam);
                //for (int i = 0; i < pArrParam.Length; i++)
                //{
                //    com.Parameters.Add(pArrParam[i]);
                //}
            }
            DataTable result = new DataTable();
            using (var da = new Microsoft.Data.SqlClient.SqlDataAdapter(com))
            {
                await Task.Run(() => da.Fill(result));
            }
            com.Dispose();
        ...
        public static async Task<T> GetEntityFromSql<T>(PTMaxstationContext pContext, string pStrSql)
        {
            T result = default(T);
            using (DataTable dt = await GetDataTable(pContext, pStrSql))
            {
                result = GetEntityFromDataTable<T>(dt);
            }
            return result;
        ...
        public static async Task ExecuteReader(
            ...
            string pStrSql,
            Action<SqlDataReader> pAcReader
        ){
            var conDb = pContext.Database.GetDbConnection();
            if (!(conDb is SqlConnection))
            {
            ...
            using (var com = new SqlCommand(pStrSql, conDb as SqlConnection))
            {
        ...
        private static async Task<GetApproverStepResult> getApproverStep2(string pStrEmpCode, PTMaxstationContext pContext = null)
        {
            if (string.IsNullOrWhiteSpace(pStrEmpCode) || pContext == null)
            {
                return null;
            }
            var pStrEmpCode2 = EncodeSqlString(pStrEmpCode);
            string strSql = @$"select top 1 * from MAS_EMPLOYEE_LEVEL(nolock)where EMP_ID = '{pStrEmpCode2}'";
            string strCon = pContext.Database.GetConnectionString(); //pContext.Database.GetDbConnection().ConnectionString;
            var dtEmpLevel = await GetDataTable(strCon, strSql);
            if(dtEmpLevel == null || dtEmpLevel.Rows.Count == 0)
            {
                return null;
            }
            var arrColName = dtEmpLevel.Columns;
            int intIndex = 1;
            string strColName = "HEAD1_ID";
            var listWorkFlolw = new List<WorkflowApprover>();
            var dr = dtEmpLevel.Rows[0];
            while (arrColName.Contains(strColName))
            {
                var strEmpId = GetString(dr[strColName]);
                if (strEmpId.Length == 0)
                {
                    continue;
                }
                var wf = new WorkflowApprover()
                {
                    stepSeq = intIndex,
                    empId = strEmpId
                };
                listWorkFlolw.Add(wf);
                strColName = $"HEAD{++intIndex}_ID";
            }
            var result = new GetApproverStepResult()
            {
                workflowApprover = listWorkFlolw.ToArray(),
                empidRequest = pStrEmpCode
            };
            if(dtEmpLevel != null)
            {
